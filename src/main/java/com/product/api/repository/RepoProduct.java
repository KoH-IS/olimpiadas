...
public interface RepoProduct extends JpaRepository<Product, Integer>{
	...
	@Modifying
	@Transactional
	@Query(value = "UPDATE product SET stock = :stock WHERE product_id = :product_id AND status = 1", nativeQuery = true)
	Integer updateProductStock(@Param("product_id") Integer product_id,
			@Param("stock") Integer stock);**/
